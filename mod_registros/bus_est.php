...
switch($_REQUEST["busqueda"]){
	case'cedula':
	$resultado=mysql_query("select a.*,b.*,c.* from paciente a, expediente b, sala c where a.ced='".quitar($_REQUEST["ced"])."' and a.ced=b.ced_paciente and b.sala=c.id_sala ",$con);
	...
	break;
	case'nombre':
	$resultado=mysql_query("select a.*,b.*,c.* from paciente a, expediente b, sala c where a.nombre like '%".strtoupper($_REQUEST["nom"])."%' and a.ced=b.ced_paciente and b.sala=c.id_sala order by ced desc",$con);
	break;
	case'apellido':
	...
	break;
	case'sala':
	$resultado=mysql_query("select a.*,b.*,c.* from paciente a, expediente b, sala c where a.ced=b.ced_paciente and b.sala='".quitar($_REQUEST["sala"])."' and b.sala=c.id_sala order by ced desc",$con);
	break;
}
