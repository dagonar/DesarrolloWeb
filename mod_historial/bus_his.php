...
<td>
	<form action="bus_his.php">
		...
		<select name="est">
			...
			<option value="0" <?php if ($_REQUEST["est"]=="0") echo "selected" ?>>AMBOS</option>
			<option value="A" <?php if ($_REQUEST["est"]=="A") echo "selected" ?>>ACTIVOS</option>
			<option value="I" <?php if ($_REQUEST["est"]=="I") echo "selected" ?>>INACTIVOS</option>
		</select>
			</td>
			<td><input type="submit" value="Buscar"></td>
		</tr>
...
FROM	paciente A,
	...
	profesional D, 
	sala E 
WHERE A.ced='".quitar($_REQUEST["ced"])."' AND A.ced=B.ced_paciente AND
...
FROM	paciente A,
	...
	profesional D, 
	sala E 
WHERE A.nombre LIKE '%".strtoupper($_REQUEST["nom"])."%' AND A.ced=B.ced_paciente AND
	B.ced_exp=C.dni_historial AND
	C.ced_prof=D.ced_prof AND B.sala=E.id_sala 
ORDER BY ced desc",$con);
	break;
	case'apellido':
	$resultado=mysql_query("
SELECT A.*,B.*,C.*,D.*,E.*
FROM	paciente A,
	...
	profesional D, 
	sala E 
WHERE A.apellido LIKE '%".strtoupper($_REQUEST["ape"])."%' AND A.ced=B.ced_paciente AND
	B.ced_exp=C.dni_historial AND
	C.ced_prof=D.ced_prof AND B.sala=E.id_sala 
ORDER BY ced desc",$con);
	break;
...
FROM	paciente A,
	...
	sala E 
WHERE 	A.ced=B.ced_paciente AND
	B.ced_exp=C.dni_historial AND B.estado_exp='".$_REQUEST["est"]."' AND 
	C.ced_prof=D.ced_prof AND B.sala=E.id_sala 
ORDER BY ced desc",$con);
	}
	break;
...
$res_patologia = mysql_query($sql_patologia, $con);
$sql_habitos = "select habitos_personales from patologia where ced='". $cedula ."'";
$res_habitos = mysql_query($sql_habitos, $con);
/************************************************/

if(mysql_num_rows($resultado)>0){
if($_REQUEST["busqueda"]=="cedula"){
...
<?php			
		...
		{
			cuadro_error("paciente: <b>".$_REQUEST["nom"]."</b> <b>".$_REQUEST["ape"]."</b> No Registrad@  <b><a href=../mod_registros/reg_est.php target=\"_self\">Registrar?</a></b>"); ///colocar un enlace para registrar a la persona
		}
}		
echo "<br>";
