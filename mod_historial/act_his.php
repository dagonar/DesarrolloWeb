...
$sql="update historial set ced_prof='".$_REQUEST["cedpro"]."',observacion='".$_REQUEST["obser"]."',diagnostico='".$_REQUEST["diagnostico"]."',tratamiento='".$_REQUEST["tratamiento"]."',fec_gen_hist='".$_REQUEST["ano"]."-".$_REQUEST["mes"]."-".$_REQUEST["dia"]."',receta='".$_REQUEST["receta"]."',ced_pac='".$_REQUEST["cedpac"]."' where dni_historial='".$_REQUEST["codexp"]."'";
...
$sql3="insert into patologia (id, ced, patologia_pac, fecha) values(NULL,'".$_REQUEST["cedpac"] ."','" .$_REQUEST["patologia"] . "',now())";
...
$result3=mysql_query("select a.*,b.* from historial a, expediente b where a.ced_pac='".quitar($_REQUEST["cedula1"])."' and a.ced_pac=b.ced_paciente ",$con);
...
if($existe==1 or $_REQUEST["existe"]==1){
///COMO YA ENCONTRO EL REGISTRO DEL PACIENTE EN LA TABLA DEL HISTORIAL CARGA EL FORMULARIO
if($cedulpro==""){$cedulpro=$_REQUEST["cedpro"];}
if($cedulpac==""){$cedulpac=$_REQUEST["cedpac"];}
if($codexp==""){$codexp=$_REQUEST["codexp"];}
if($existe==""){$existe=$_REQUEST["existe"];}
if($estado==""){$estado=$_REQUEST["estado"];}
if($obser==""){$obser=$_REQUEST["obser"];}
if($dia==""){$dia= date('d');}
if($mes==""){$mes= date('m');}
if($ano==""){$ano= date('Y');}
if($diagnostico==""){$diagnostico=$_REQUEST["diagnostico"];}
if($tratamiento==""){$tratamiento=$_REQUEST["tratamiento"];}
if($receta==""){$receta=$_REQUEST["receta"];}
...
<?php
if($cedulpac!=""){
$result=mysql_query("select a.*,b.* from paciente a, expediente b where a.ced=b.ced_paciente and b.ced_paciente='".quitar($cedulpac)."' ",$con);
...
$res_habitos = mysql_query($cmd_habitos, $con);
if(mysql_num_rows($result) == 1){
