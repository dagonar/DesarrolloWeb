...
if (strtolower($_REQUEST["acc"])=="modificar"){
	...
	if (@mysql_query("update usuarios set tipo='".$_REQUEST["tipo"]."',nombre='".strtoupper(htmlentities($_REQUEST["nombre"]))."',password='".htmlentities($pass)."' where id_usu='".$_REQUEST["iduser"]."' ",$con)){
		echo"<br /><br />";
...
switch ($_REQUEST["modificacion"]){
	case'login':
	$result=mysql_query("SELECT * FROM usuarios WHERE login='".$_REQUEST["login"]."'");
	break;
	case'nombre':
	$result=mysql_query("SELECT * FROM usuarios WHERE nombre like '%".strtoupper($_REQUEST["nombre"])."%'");
	break;
	}
...
<input type="hidden" name="iduser" value="<?php echo mysql_result($result,0,"id_usu");?>">
...
<tr>
	...
	<td><input type="text" name="nombre" value="<?php echo mysql_result($result,0,"nombre"); ?>" size="45"></td>
</tr>
<tr>
	<td class="tdatos">Login</td>
	<td><input type="text" name="login" value="<?php echo mysql_result($result,0,"login"); ?>" readonly size="45"></td>
</tr>
<tr>
	<td class="tdatos">Password</td>
	<td><input type="password" name="pass" value="<?php echo mysql_result($result,0,"password"); ?>" size="45"></td>
</tr>
<tr>
	<td class="tdatos">Repetir Password</td>
	<td><input type="password" name="pass2" value="<?php echo mysql_result($result,0,"password"); ?>" size="45"></td>
</tr>
<tr>
